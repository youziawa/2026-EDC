...
extern s16 TMD_Val[4]; //自主开发闭环控制使用的目标速度值
...
//		}
		...
		//实时XYZ-YAW期望速度(实时控制帧)
		rt_tar.st_data.yaw_dps = TMD_Val[3];  //航向转动角速度，度每秒，逆时针为正
		rt_tar.st_data.vel_x = TMD_Val[0];    //头向速度，厘米每秒
		rt_tar.st_data.vel_y = TMD_Val[1];    //左向速度，厘米每秒
		rt_tar.st_data.vel_z = TMD_Val[2];	 //天向速度，厘米每秒
		//########################################################################
		//=====
