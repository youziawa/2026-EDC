...
// 通道8高值，向下掰
#define UN_LOCK (rc_in.rc_ch.st_data.ch_[ch_8_aux4] > 1700 && rc_in.rc_ch.st_data.ch_[ch_8_aux4] < 2200)
#define LOCK (rc_in.rc_ch.st_data.ch_[ch_8_aux4] > 800 && rc_in.rc_ch.st_data.ch_[ch_8_aux4] < 1200)
...
// 摇杆解锁条件requirement
#define STICKS_UNLOCK_REQ (UN_LOCK) // 由原来#define STICKS_UNLOCK_REQ (TOE_OUT || TOE_IN)更改
// 摇杆上锁条件
#define STICKS_LOCK_REQ (LOCK)
// 解锁持续时间,毫秒
...
{
	// 判断yaw摇杆是否大致回中
	if ((rc_in.rc_ch.st_data.ch_[ch_4_yaw] > 1400 && rc_in.rc_ch.st_data.ch_[ch_4_yaw] < 1600)) // 左摇杆左右方向居中
	{
		sti_fun.stick_mit_pos = 1; // 在中位（这里说的“在不在中位”指摇杆左右方向是否回到大致中心位置。回中后才允许触发一次解锁/上锁动作，防止连击或误操作）
		unlock_lock_flag = 1;	   // 回中以后才能执行一次解锁或者上锁
	}
	else
	{
		sti_fun.stick_mit_pos = 0; // 不在中位
	}
	// 标记预备上锁的动作
	if (rc_in.rc_ch.st_data.ch_[ch_3_thr] < 1200 && (sti_fun.stick_mit_pos == 0)) // 油门低且yaw不在中位
	{
		sti_fun.pre_locking = 1; // 预备上锁
	}
	else
	{
		sti_fun.pre_locking = 0; // 取消预备上锁
	}
	// 解锁
	if (unlock_lock_flag == 1) // 执行条件
	{
		if (UNLOCK_STATE == 0) // 当前为上锁状态
		{
			if (STICKS_UNLOCK_REQ) // 满足解锁条件,通道8向下掰
			{
				if (time_dly_cnt_ms < UNLOCK_HOLD_TIME_MS) // 持续时间未到
				{
					time_dly_cnt_ms += *(dT_s) * 1e3f; // 累加时间
				}
				else // 持续时间到，执行解锁
				{
					FC_Unlock(); // 解锁
			...
			{
				time_dly_cnt_ms = 0; // 清零时间
			}
		}
		else if (UNLOCK_STATE == 1) // 当前为解锁状态
		{
			if (STICKS_LOCK_REQ) // 满足上锁条件
			{
...
void LX_Cali_Trig_Check() // 摇杆触发校准检测
{
	static u8 cali_f;
	// 为上锁状态才执行
	if (UNLOCK_STATE == 0)
	{
		// 执行条件
		if (STICKS_CALI_HOR_REQ) // 摇杆触发校准水平条件
		{
			// 标记只执行一次
			if (cali_f == 0)
			{
