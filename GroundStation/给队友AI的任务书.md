...
1. 节点地址固定 `SRC=0x03`，只向飞机主控 `DST=0x02`发送本地测量。
2. 10～20 Hz连续发送 `VISION_PIXEL 0x40`：`valid/kind/confidence/dx_px/dy_px/`
   `radius_px/vision_mode`，长度12字节。它不发送厘米距离、FRD坐标或飞控动作。
3. `dx_px`右方为正、`dy_px`下方为正；F4读取飞控实际高度后，负责换算为内部FRD
   距离和控制误差。
4. 无目标时仍周期发送，`valid=0`且`dx_px/dy_px/radius_px`清零；状态可为LOST。
5. `0x40`不得转发到小车、地面站或凌霄飞控；`0x41`当前保留，禁止自行发送。
6. 视觉只输出测量，不直接控制飞控、投放器或状态机。

完成标准：人工向左/右、上/下移动目标时`dx_px/dy_px`符号正确；遮挡超过300 ms后
飞机按视觉无效处理，不继续使用旧误差。
